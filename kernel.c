...
void Serd_OS_main()
{
    ...
    uart_writeText("Waiting for start\n");
    while (!getUart())
    ...
    while (lives > 0 && bricks > 0)
    {
        ...
        {
            if (ch == 'l')
                if (paddle->x + paddle->width + (paddle->width / 2) <= WIDTH - MARGIN)
                    moveObject(paddle, paddle->width / 2, 0);
            if (ch == 'h')
                if (paddle->x >= MARGIN + (paddle->width / 2))
                    moveObject(paddle, -(paddle->width / 2), 0);
        }
        uart_loadOutputFifo();
        ...
        if (foundObject)
        {
            if (foundObject == paddle)
            {
                ...
                if (ball->x + ball->width + velocity_x == paddle->x || ball->x + velocity_x == paddle->x + paddle->width)
